...
# Function to generate content using the selected AI model
def generate_content(code_content, user_prompt, previous_messages, selected_model, max_tokens=1500, temperature=0):
    ...
    elif 'IBM' in selected_model:
        # For IBM models
        response = generate_content_ibm(full_prompt, max_tokens=max_tokens)
        if response is None:
            return "Error generating content with IBM's model.", message_history
    else:
...
def generate_content_huggingface(prompt, model_name="generate_content_ibm", max_tokens=1500):
    generator = pipeline('text-generation', model=model_name)
    response = generator(prompt, max_length=max_tokens)[0]
    return response['generated_text']

# Function to generate content using IBM's model
def generate_content_ibm(prompt, max_tokens=1500):
    try:
        # Tokenize the text
        ...
        output = model.generate(**input_tokens, max_length=max_tokens)
        # Decode output tokens into text
        output_text = tokenizer.batch_decode(output, skip_special_tokens=True)
        return output_text[0]
    except Exception as e:
        st.error(f"An error occurred while generating content with IBM's model: {e}")
        return None
...
model_options = {
    ...
    'IBM Granite': 'ibm-granite/granite-3b-code-base'
}
